...
async function openSiteInBrowser(query, newTab, requestedOutputLanguage, options = {}) {
  ...
  const outputLanguage = normalizeOutputLanguage(requestedOutputLanguage || 'en');
  const outputMessagesReady = ensureOutputMessages(outputLanguage);
  let currentHostname = hostnameFromUrl(sourceTabUrl);

  if (!currentHostname || isNavableExtensionUrl(sourceTabUrl)) {
    try {
      const [activeTab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
      const activeTabUrl = String((activeTab && (activeTab.url || activeTab.pendingUrl)) || '');
      if (activeTabUrl && !isNavableExtensionUrl(activeTabUrl)) {
