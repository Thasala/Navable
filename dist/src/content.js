...
(function () {
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      ...
      if (msg && (msg.type === 'VOICE_COMMAND' || msg.type === 'navable:voiceTranscript')) {
        handleVoiceCommandText(msg.text || '', msg.language || '').then(function () {
          sendResponse && sendResponse({ ok: true });
        }).catch(function (err) {
  ...
  async function handleVoiceCommandText(text, detectedLanguage) {
  ...
  async function handleTranscript(text, detectedLanguage, provider) {
    return handleVoiceCommandText(text, detectedLanguage, provider);
  }
